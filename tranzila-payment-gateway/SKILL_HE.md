...
תבדקו את שדה `Response` בכל תוצאת עסקה. `000` פירושו אושר -- כל דבר אחר הוא שגיאה.

שגיאות נפוצות לטיפול בקוד:

| קוד | משמעות | פעולה מומלצת |
|------|---------|-------------|
| 004 | כרטיס סורב | תבקשו מהמשתמש לנסות כרטיס אחר |
| 036 | כרטיס פג תוקף | תבקשו מהמשתמש לעדכן פרטי כרטיס |
...
| 200 | שגיאת אפליקציה | נסו שוב; אם נמשך, תבדקו פרמטרים |
| 900 | אימות 3DS נכשל | נסו ללא 3DS או תבקשו מהמשתמש לאמת |
...
4. אימות: תבדקו Response=000 לתשלום Bit מוצלח
...
פתרון: ודאו שכל הפרמטרים הנדרשים קיימים: supplier, TranzilaPW, sum, ccno (או TranzilaTK), expdate. תבדקו ששמות הפרמטרים מדויקים (case-sensitive).
