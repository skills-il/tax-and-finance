...
## מלכודות נפוצות
- סוכנים משתמשים לעתים בשיעור מע"מ ישן של 17% בחישובים. שיעור המע"מ הנוכחי בישראל הוא 18% (מינואר 2025). כל נוסחאות המע"מ חייבות להשתמש ב-0.18, לא 0.17.
- פורמט תאריך ישראלי הוא DD/MM/YYYY, לא MM/DD/YYYY. סוכנים ברירת מחדל לפורמט אמריקאי, מה שגורם לרישום תאריכים שגוי (למשל 03/01/2026 אומר 3 בינואר בישראל, לא 1 במרץ).
- Google Workspace CLI דורש אימות OAuth בצד השרת. סוכנים עלולים לנסות להשתמש ב-API keys או service accounts, שלא נתמכים על ידי gws auth login.
- סוכנים עלולים לסווג הוצאות רכב כמוכרות ב-100%, אבל חוק המס הישראלי מגביל ניכוי הוצאות רכב ל-45% או סכום קבוע. זו טעות נפוצה בסיווג הוצאות.
