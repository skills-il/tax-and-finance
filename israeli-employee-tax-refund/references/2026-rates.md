# 2026 Rates Snapshot for Employee Tax Refunds

This file is a cached snapshot of the values used by the skill. Re-verify against Kol-Zchut before quoting.

## Credit-point value

| Period | Value |
|--------|-------|
| Monthly | 242 ₪ |
| Annual | 2,904 ₪ |

Source: Kol-Zchut "נקודות זיכוי ממס הכנסה" page.

## Monthly tax brackets

| Monthly salary | Annual salary | Rate |
|----------------|---------------|------|
| Up to 7,010 ₪ | Up to 84,120 ₪ | 10% |
| 7,011 - 10,060 ₪ | 84,121 - 120,720 ₪ | 14% |
| 10,061 - 19,000 ₪ | 120,721 - 228,000 ₪ | 20% |
| 19,001 - 25,100 ₪ | 228,001 - 301,200 ₪ | 31% |
| 25,101 - 46,690 ₪ | 301,201 - 560,280 ₪ | 35% |
| 46,691 ₪ and above | 560,281 ₪ and above | 47% (Section 121 ITO, top employment bracket) |
| Plus mas yesafim | Annual income above 721,560 ₪ | Additional 3% surtax (Section 121B ITO), applied on top of 47% |

For prior tax years, look up the brackets that applied to that year on kolzchut.org.il rather than extrapolating the 2026 table.

## Reserve duty credit points (Section 39B, Amendment 283 התשפ"ו-2025)

| Days in tax year | Points | Annual value |
|------------------|--------|--------------|
| 30 - 39 | 0.5 | 1,452 ₪ |
| 40 - 49 | 0.75 | 2,178 ₪ |
| 50 - 54 | 1.0 | 2,904 ₪ |
| Each additional 5 days | +0.25 | +726 ₪ |
| Maximum | 4.0 | 11,616 ₪ |

Points are realized in the tax year AFTER the service.

## Section 46 donations

Both the minimum and the maximum are index-adjusted every year, so a claim for an earlier year must use that year's figures, not 2026's.

| Tax year | Minimum donation to qualify | Maximum eligible for credit |
|---|---|---|
| 2022 | 190 ₪ | 9,517,000 ₪ |
| 2023 | 200 ₪ | 10,019,808 ₪ |
| 2024 | 207 ₪ | 10,354,816 ₪ |
| 2025 | 207 ₪ | 10,354,816 ₪ |
| 2026 | 207 ₪ | 10,354,816 ₪ |

The ITA no longer serves the 2020 and 2021 deductions booklets, so those two years' minimums are not verified here. Look them up in that year's booklet before disqualifying a small donation from 2020 or 2021.

| Parameter | 2026 value |
|-----------|------------|
| Maximum eligible, alternative limb | 30% of taxable income (lower of the two applies) |
| Credit rate | 35% of donation |
| Receipt format | Original, certified copy, or electronic (מסמך ממוחשב). Always verify the institution's Section 46 approval was active in the year of the donation. |

## Retroactive refund window (Section 160 ITO)

| Tax year | Deadline |
|----------|----------|
| 2020 | 31.12.2026 |
| 2021 | 31.12.2027 |
| 2022 | 31.12.2028 |
| 2023 | 31.12.2029 |
| 2024 | 31.12.2030 |
| 2025 | 31.12.2031 |

## Statutory processing time

Refund must be paid within one year from the assessment date, or two years from the end of the tax year, whichever is later. Late refunds accrue הצמדה (CPI linkage) plus 4% annual interest.

## Section 9(5): Disability exemption ceilings (2026, in force through 2027)

| Scenario | 2026 ceiling (NIS/year) |
|---|---|
| Disability 365 days or more, earned income (standard case) | 445,200 |
| Disability 365 days or more, income is from a pension under חוק הנכים or חוק נפגעי פעולות איבה | 684,000 |
| Disability 185-364 days, earned income | 81,960 |
| Non-personal-labor (passive) income, any qualifying duration | 81,960 |
| Interest income from compensation funds (קרן פיצויים), 365 days or more | 337,200 |
| Blind individuals (עיוורים) | Same ceilings as 100% medical disability |

Qualifying threshold: 100% medical disability, blindness, OR 90%+ via the multi-organ-injury calculation (פגיעה באיברים שונים). Under 185 days does not qualify. Ceiling-based, not rate-based: income up to the cap is fully exempt; income above is taxed normally.

## Section 35: Olim chadashim credit-point schedule

For olim arriving 1.1.2022 or later (post-Amendment 262 of 7.5.2022):

| Period from aliyah | Points/month | Annual rate |
|---|---|---|
| Months 1-12 | 1/12 | 1 |
| Months 13-30 | 1/4 | 3 |
| Months 31-42 | 1/6 | 2 |
| Months 43-54 | 1/12 | 1 |

Total: 8.5 credit points across 54 months (4.5 years).

For pre-2022 arrivals: 4.5 / 2 / 1 across 18 / 12 / 12 months (total 7.5 points, 42 months).

Returning residents (תושב חוזר) generally do NOT qualify for Section 35. Most returnees use the 10-year foreign-income exemption under Section 14 instead. Veteran returning residents (תושב חוזר ותיק, after 10+ years abroad) have a separate Milchan-amendment benefit track that is out of scope here.

## Section 40g: Academic credit-points schedule

| Degree + graduation cohort | Points |
|---|---|
| Bachelor's, 2014-2022 graduates | 1 point for one tax year (claimant's choice: graduation year or year after) |
| Bachelor's, 2023+ graduates | 1 point per study year, capped at 3 years (so a 3-year B.A. = 3 points spread over 3 years) |
| Master's, 2014-2022 graduates | 0.5 point for one tax year |
| Master's, 2023+ graduates | 0.5 point per year for 2 years |
| Professional degrees with mandatory internship (e.g., medicine) | Same bachelor's points but the claimant may defer the start to the year after internship completion |

## Section 11: Yishuv mutav (preferred-locality) credit, 2026

Eligibility requires the taxpayer's centre of life in the locality for at least 12 continuous months, and the credit applies only to הכנסה חייבת מיגיעה אישית (earned income). The rate and the annual ceiling are set PER LOCALITY, not nationally.

The authoritative table is chapter ח of the Israel Tax Authority deductions booklet for 2026, which lists every eligible locality with its 2026 rate and 2026 ceiling. Localities added, or whose rate or ceiling changed, are printed in bold there. Read the value for the user's specific locality from that table. Do not interpolate from a nearby locality.

The ITA calls this יישוב מוטב (yishuv mutav); older material and parts of this skill say yishuv mezakeh. They are the same benefit under section 11 ITO.

The 2026 table published by the ITA has **15 rate/ceiling combinations**. The rate does NOT determine the ceiling: 12% appears against four different ceilings and 14% against four. Never read this as a lookup ladder and never infer a ceiling from a rate. Read BOTH values for the user's own locality from chapter ח of the booklet.

Official 2026 combinations (chapter ה, table 1 of the ITA deductions booklet):

| Rate | Annual ceiling (NIS) |
|------|----------------------|
| 7% | 146,640 |
| 10% | 180,000 |
| 12% | 186,000 |
| 12% | 186,600 |
| 12% | 213,240 |
| 12% | 226,560 |
| 14% | 180,000 |
| 14% | 213,240 |
| 14% | 219,960 |
| 14% | 259,920 |
| 16% | 226,560 |
| 18% | 245,400 |
| 18% | 259,920 |
| 20% | 259,920 |
| 20% | 267,840 |

Representative localities: ראש פינה / רגבה / צורית / קדרים at 7%; קצרין / רביבים / פרוד / קליה at 10%; צפת / קריית ארבע / קטורה / צופר at 12%; רהט at 16%; קריית שמונה at 20% / 259,920 and שדרות at 20% / 267,840.

Localities ADDED for 2026: אדוריים, קדם ערבה (10%, ceiling 180,000), יונדב (7%, ceiling 146,640), בתרון (12%, ceiling 213,240), נוף הגליל and אשקלון. Added or changed rows are printed in bold in chapter ח of the booklet.

Three regimes new for 2026, each a הוראת שעה running 2026-2029, each amending section 11 ITO:

- **חוק הטבות מס ליישובים (תיקוני חקיקה), התשפ"ו-2026**: changed rates and ceilings for part of the חבל תקומה localities and added the city of **אשקלון** to the preferred-locality list. Section 11(ב2) caps a חבל תקומה-adjacent resident at 14% up to a 180,000 NIS ceiling.
- **חוק עידוד יישוב עירוני מעורב (הוראת שעה), התשפ"ו-2026**: a resident of a mixed urban preferred locality gets 12% up to a ceiling of 226,560 NIS (section 11(ב3)). **נוף הגליל** was added under this order.
- **חוק הטבות מס ליישוב אזור קו עימות מזרחי (הוראת שעה), התשפ"ו-2026**: eastern-confrontation-line localities enter at the section 11(ב)(1) rate and ceiling (section 11(ב4)).

Where more than one limb applies the taxpayer chooses one, not both. Section 11(ו) obliges the ITA director to publish the preferred-locality list and each locality's total score by 15 January of every tax year, so the current-year list is always the operative source.

Two separate regimes that are NOT in that table:

- **Eilat and Chevel Eilot**: 10% on earned income produced in the area, up to 268,560 NIS, under section 11 of the Eilat Free Trade Zone Law (not section 11 ITO).
- **Security forces**: 5% of the special salary, up to 178,320 NIS, under section 11(c) ITO.

Evacuee note: residents of localities evacuated under government decisions following the war keep the benefit while the locality remains defined as evacuated, subject to the Tax Authority's employer guidance. Follow the current ITA guidance rather than assuming the status persists.

## Aliya exemption temporary order (חוק עידוד עלייה לישראל וחזרה אליה (הוראת שעה), התשפ"ו-2026)

Enacted as chapter ד of חוק ההתייעלות הכלכלית 2026 and in force from 31.3.2026. It grants a full income-tax **exemption** (not a credit) on הכנסה מיגיעה אישית (business and salary income) produced in Israel. The ITA states expressly that it is given **in addition to** every other benefit available to olim and returning residents, so it stacks on top of the section 35 credit points rather than replacing them.

Who qualifies: an oleh chadash who became an Israeli resident for the first time, or a תושב חוזר ותיק (a returning resident who was a foreign resident for at least 10 consecutive years), where residency began between **05.11.2025 and 31.12.2026**.

Annual exemption ceilings, set by section 2(a) of the temporary order, for tax years 2026 through 2030:

| Tax year | Ceiling, income NOT from a relative | Ceiling, income only from a relative | Ceiling, mixed |
|---|---|---|---|
| 2026 | 600,000 ₪ | 140,000 ₪ | 600,000 ₪ |
| 2027 | 1,000,000 ₪ | 140,000 ₪ | 600,000 ₪ |
| 2028 | 1,000,000 ₪ | 140,000 ₪ | 1,000,000 ₪ |
| 2029 | 350,000 ₪ | 140,000 ₪ | 350,000 ₪ |
| 2030 | 150,000 ₪ | 150,000 ₪ | 150,000 ₪ |

"Relative" covers children, a spouse and their children, siblings and their children, parents and the parents' siblings, grandparents, and any company in which the claimant or a relative holds 25% or more. With both kinds of income, each is exempt up to its own ceiling but the combined exemption cannot exceed the higher of the two.

Other operative rules:

- In 2026 the ceiling is pro-rated to the part of the year from the date residency began. An oleh who arrived on 01.07.2026 gets 50%, so 300,000 ₪.
- The benefit **sunsets after tax year 2030**. It is a temporary order, not a permanent amendment to the Ordinance.
- Because it starts with 2026 income, it cannot yet be taken prospectively through payroll. A salaried claimant realises it by filing a **refund request after tax year 2026 closes** (that is, from January 2027), which is exactly this skill's workflow.
- Anti-abuse rule: someone who ceases to be an Israeli resident during 2028 or 2029 and spends fewer than 75 days in Israel in one of those years loses the exemption entirely, including retroactively for 2026 and 2027.
- A circular setting out the full conditions was still pending publication when the 2026 deductions booklet went out. Check for it before relying on edge-case treatment.

## Section 10: shift-work tax benefit (עבודה במשמרות)

Section 10 ITO empowers the Finance Minister to set a tax benefit for second- and third-shift income, and the operative rates live in תקנות מס הכנסה (שיעור המס על הכנסה בעד עבודה במשמרות), התשמ"ז-1986. Publication 11801 of 23.03.2025 extended those regulations to **31.12.2026**.

| Parameter | 2026 value |
|---|---|
| Credit rate | 15% of the shift pay |
| Maximum credit, section 10(ב) | 12,540 ₪ per tax year |
| Income ceiling, section 10(ג) | 143,040 ₪ per tax year |

Eligibility is narrow. Section 10(ב) confines the benefit to **industrial plants whose main activity in the tax year is productive activity** within the meaning of חוק עידוד התעשיה (מסים), התשכ"ט-1969, as designated by the Finance Minister. A shift worker in retail, security, or healthcare is outside it. Earlier years use their own figures: for 2022 the maximum credit was 11,520 ₪ against an income ceiling of 131,520 ₪.

## Credit-point entitlements the employer rarely applies at source

Every row below is claimable on the annual refund and appears as a claim box in part ז of Form 101.

| Entitlement | Points | Anchor |
|---|---|---|
| Single parent (הורה במשפחה חד-הורית) not entitled under section 37 | The full child schedule (2.5 birth year, 4.5 to age 2, 3.5 at 3, 2.5 at 4 and 5, 2 from 6 to the year before 18, 0.5 in the year the child turns 18) | s.40(ב)(1) |
| Child of a single parent (ילד להורה אחד) where that parent already claims the row above | One ADDITIONAL point | s.40(ב)(1ב) |
| Parents living apart: the parent entitled under s.40(ב)(1) | One additional point | s.40(ב)(2) |
| Parents living apart, maintenance split: the parent NOT entitled under s.40(ב)(1) | One point, or a fraction matching their share of the child's upkeep | s.40(ב)(2) |
| Divorced person paying maintenance to a former spouse and married to someone else | 1 point | s.40א |
| Individual aged 16 or 17 | 1 point | s.40ב |
| Completion of vocational studies with a תעודת מקצוע | 1 point per study year, capped at 3 tax years, from the year after studies end | s.40ד |

Two rules that decide which row applies:

- The half point in the child's **maturity year** (the tax year the child turns 18) belongs to the mother under s.66(ג)(4)(א) and to a single parent under s.40(ב)(1). The father's schedule at s.66(ג)(5)(ג) runs only to the tax year **preceding** the maturity year, with no maturity-year point.
- Vocational studies (s.40ד) and an academic degree (s.40ג) are alternatives, not cumulative. Section 40ה gives the claimant the choice; compute both and keep the larger. Vocational studies means at least 1,700 study hours and a certificate recognised by a government ministry.

## Default credit-point allotment for a salaried resident

| Recipient | Points per year |
|-----------|-----------------|
| Israeli resident (base) | 2.25 |
| Female resident (additional 0.5) | 2.75 |
| Each child up to age 5 (2024+ schedule, per parent) | 2.5 in birth year, 4.5 age 1, 4.5 age 2, 3.5 age 3, 2.5 age 4, 2.5 age 5 |
| Each child age 6 to the year before 18 (to mother, post-2022) | 2 |
| Each child in the tax year they turn 18 (mother, or a single parent) | 0.5. The father's schedule stops at the year before, with no maturity-year point |
| Each child age 6 to the year before 18 (to father) | 1 |
| Each child age 6-17 (pre-2022) | 1 (mother) / 0 (father) |
| Single parent (הורה במשפחה חד-הורית) not entitled under s.37 | The full child schedule above (2.5 / 4.5 / 3.5 / 2.5 / 2 / 0.5), NOT the father's reduced one |
| Child of a single parent (ילד להורה אחד) | 1 additional point to that parent, on top of the row above |
| Parents living apart: the parent entitled under s.40(ב)(1) | 1 additional point |
| Parents living apart, upkeep split: the parent NOT entitled under s.40(ב)(1) | 1 point, or the fraction matching their share of the upkeep |
| Divorced, paying maintenance to a former spouse, and remarried (s.40א) | 1 |
| Individual aged 16 or 17 (s.40ב) | 1 |
| Vocational certificate, תעודת מקצוע (s.40ד) | 1 point per study year, capped at 3 tax years, from the year after studies end. Alternative to the academic points, not cumulative; s.40ה lets the claimant pick the larger |
| New immigrant, post-2022 arrival (s.35) | 1 point year 1, 3 points years 1.5-2.5, 2 points year 3, 1 point year 4 |
| Master's graduate (2023+) | 0.5 point per year for 2 years after graduation |
| Bachelor's graduate (2023+) | 1 point per study year, capped at 3 years |
| Discharged soldier / national-service graduate (s.39א) | 2 points/year for full service (23+ months men, 22+ months women), 1 point/year for shorter service, over the 36 months from the month after discharge |
| Parent of a נטול יכולת child (s.45(א)) | 2 points/year per child, including the spouse's child; claimed on Form 116א, never applied at source |

