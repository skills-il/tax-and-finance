...
- CreditType values limited to what a vendor publishes: `1`, `6`, `8`. Source: Tranzila's iframe parameter table (all three); Cardcom v11 schema (`CreditType`, default `1`, names `6`). Values 2/3/4/5/9 must be explicitly warned against, since Tranzila answers them with an unauthorized-credit-type error.
