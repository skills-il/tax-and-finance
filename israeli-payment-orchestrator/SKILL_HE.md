...
| Tranzila | REST/Form POST | רגיל, קרדיט, מועדון | כן | כן (redirect) | לא | 0.5-0.7% |
...
| התאמת פיצ'ר | גבוהה | נתב לפי פיצ'רים נדרשים | תשלומי מועדון (cred_type 9) ל-Cardcom, Pelecard או Tranzila |
...
| Meshulam (Grow) - Reference | https://developers.grow.business/reference/overview | בסיס פרודקשן secure.meshulam.co.il (אל תשתמשו ב-sandbox.meshulam.co.il בפרודקשן) |
...
פתרון: תבדקו יכולות שער בטבלת שלב 2. תשלומי מועדון (Shva CreditType 9) נתמכים ב-Cardcom, ב-Pelecard וב-Tranzila (מגבלת רשת הכרטיסים על cred_type 9 עבור Visa/Diners חלה באופן זהה בכל השערים, ולכן אינה מגבלה של שער מסוים). תנתבו לשער שבו המסוף מוגדר לתוכנית המועדון.
