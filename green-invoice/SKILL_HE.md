...
## מלכודות נפוצות
- סוכנים משתמשים לעתים בסוג מסמך 305 (חשבונית מס) במקום 320 (חשבונית מס / קבלה), שהוא הסוג הנכון לרוב עסקאות B2C בישראל כשהתשלום מתקבל מיידית.
- שיעור המע"מ בתגובות ה-API משקף את הגדרת סוג העסק, לא ערך קבוע. סוכנים לא צריכים לדרוס חישובי מע"מ אלא אם יש פריטים עם מע"מ מעורב.
- טוקני JWT של חשבונית ירוקה פגים מדי פעם. סוכנים עלולים לשמור טוקן במטמון ולהשתמש בו מחדש בין סשנים בלי לרענן, מה שמוביל לשגיאות 401.
- סוכנים מעבירים לפעמים vatType: 1 ברמת המסמך בחושבם שזה אומר "כולל מע"מ", אבל בפועל זה אומר "פטור ממע"מ". יש להשתמש ב-vatType: 0 להתנהגות מע"מ רגילה.
- ה-API שינה את המיתוג מ"חשבונית ירוקה" ל"מורנינג" אבל הדומיין נשאר api.greeninvoice.co.il. סוכנים עלולים לנסות להשתמש ב-api.morning.co.il שלא קיים.
