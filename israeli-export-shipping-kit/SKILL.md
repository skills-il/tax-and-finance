...
- An exporter is permanently in a VAT refund position (zero output VAT, real input VAT), so the input side is where the money is, and Israel's allocation-number rule bites there and ONLY there. **The rule runs one way for an exporter, and getting the direction wrong costs money in both.** On the INPUT side, a tax invoice from an Israeli supplier needs a `מספר הקצאה` before you can deduct the VAT on it. These are ordinary domestic taxable supplies, so the zero-rate carve-out does NOT protect them: freight forwarding, customs brokerage, packaging and crating invoices land above the threshold routinely, and the deduction is disallowed without a number. On the OUTPUT side, your own export invoices are zero-rated and are expressly outside the requirement under s.47(א2)(1), which applies only `לעניין חשבונית מס שהוצאה בשל עסקה שהמס שחל לגביה אינו בשיעור אפס`. Never flag your own export invoice for a missing allocation number, however large. Applying the rule symmetrically is the classic exporter error.
  The threshold is keyed to the INVOICE's own date, not today's, so a supplier invoice from an earlier year is tested against its own band: nothing before May 2024, then NIS 25,000 from May 2024, NIS 20,000 from January 2025, NIS 10,000 from January 2026 and NIS 5,000 from June 2026. Two further points from the statute. The loss of YOUR deduction is s.38(א1), which has NO buyer-request condition, so do not wait to see whether anyone demanded a number: `לא יותר ניכוי מס התשומות הכלול בחשבונית מס שסכומה, בלא המס, עולה על 5,000 שקלים חדשים (מינואר 2026 ועד מאי 2026: 10,000 שקלים חדשים) ושאינה כוללת מספר שהקצה לה המנהל`. That demand belongs to the SELLER'S duty under s.47(א2)(1) and has nothing to do with whether you can deduct. And the subsection says `עולה על` (EXCEEDS), so a supplier invoice sitting exactly on a band figure is outside the rule. Check for the allocation number when the invoice arrives, not at the end of the reporting period, because chasing a supplier for it months later is how a refund claim gets cut.
