...
def render_gap_chart(
    ...
    for i, p in enumerate(pairs):
        ...
        bars.append(
            ...
            f'fill="var(--{role})" fill-opacity="{op}"/>'
        )
        plabels.append(
    ...
    trows = "".join(
        ...
        f"<td>{'yes' if p.flagged else 'no'}</td></tr>"
        for p in pairs
    )
...
<body><div class="wrap">
  ...
  <div id="tableview"><table><thead><tr><th>Pair</th><th>Gap %</th><th>TASE (ILS)</th>
    <th>US→ILS</th><th>Flagged</th></tr></thead><tbody>{trows}</tbody></table></div>
  {una_html}
</div>
<script>
...
function move(ev){{const r=svg.getBoundingClientRect(),sy=(ev.clientY-r.top)/r.height*VH;
  ...
  const lines=[['Gap',row.gap],['TASE (ILS)',row.tase],['US to ILS',row.us],['Status',row.flag]];
